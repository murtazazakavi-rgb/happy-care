...
/* ============================================================
   ...
   ============================================================ */
const DB={
  key:'happycare_v1', data:null, mem:false, isCloud:false, syncEnabled:true,
  ...
  async loadServer() {
    try {
      ...
      console.error('Failed to connect to backend database, falling back to local browser storage:', e);
      this.loadLocal();
    }
    if (!this.data) {
      this.data = { _seq: {} };
      this.tables.forEach(t => this.data[t] = []);
    }
    if (!this.data._seq) this.data._seq = {};
  },
  ...
  save() {
    ...
    try {
      ...
      this.mem = true;
      toast('Storage blocked — running in memory only', 'err');
    }
  },
  ...
  async saveAllToServer() {
    ...
    if (indicator) {
      indicator.style.background = 'var(--peach)';
      indicator.title = 'Saving all data to Neon DB...';
    }
    try {
      const res = await fetch('/api/db', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'save_all', data: this.data }),
      });
      const result = await res.json();
      if (result.error) {
        toast('Sync failed: ' + result.error, 'err');
        if (indicator) indicator.style.background = 'var(--urgent)';
      } else {
        if (indicator) {
          indicator.style.background = 'var(--ok)';
          indicator.title = 'Synced with Neon DB';
        }
      }
    } catch (e) {
  ...
  async syncCloud(action, table, id, data) {
    if (!this.isCloud || !this.syncEnabled) return;
    const indicator = document.getElementById('cloud-sync-indicator');
    if (indicator) {
      indicator.style.background = 'var(--peach)';
      indicator.title = 'Saving changes to Neon DB...';
    }
    try {
      const res = await fetch('/api/db', {
        method: 'POST',
      ...
      if (result.error) {
        ...
        toast('Sync warning: ' + result.error, 'err');
        if (indicator) indicator.style.background = 'var(--urgent)';
      } else {
        if (indicator) {
          indicator.style.background = 'var(--ok)';
          indicator.title = 'All changes saved to Neon DB';
        }
      }
    } catch (e) {
      console.error('Network sync error:', e);
      toast('Network sync error: modification saved locally', 'err');
      if (indicator) indicator.style.background = 'var(--urgent)';
    }
  },

  next(t) {
    this.data._seq[t] = (this.data._seq[t] || 0) + 1;
    return this.data._seq[t];
  },
  ...
  insert(t, o) {
    o.id = o.id || this.next(t);
    this.data[t].push(o);
    this.save();
    this.syncCloud('insert', t, o.id, o);
    return o;
  },
  ...
  remove(t, id) {
    ...
    this.save();
    this.syncCloud('remove', t, id, null);
  },
  
  byId(t, id) {
    return this.data[t].find(x => x.id === id);
  },
  
  find(t, fn) {
    return this.data[t].find(fn);
  },
  
  filter(t, fn) {
    return this.data[t].filter(fn);
  },
...
function seed(){
  ...
  const firsts=['Yusuf','Fatema','Mustafa','Zainab','Husain','Sakina','Taher','Zahra','Burhanuddin','Amatullah','Ali','Ruqaiya','Abdeali','Arwa','Murtaza','Idris','Khadija','Shabbir','Rashida','Mohammed','Mariam','Khozema','Tasneem','Hatimi','Nafisa'];
  const lasts=['Contractor','Zakavi','Kapadia','Chunawala','Bharmal','Fakhri','Rampura','Vasanwala','Tambawala','Merchant','Jiwanji','Godiwala','Sodawala','Lokhandwala','Halwadwala','Badri','Saifee','Kanchwala','Mithaiwala','Kagdi'];
  const categories=['Toddlers (2-3)','Preschool (3-4)','Kindergarten (4-5)','Prep (5-6)'];
  const categoriesBrief=['2–3','3–4','4–5','5–6'];
  ...
  const ph1=DB.insert('phases',{
    programId:prog.id, name:'Phase 1', startDate:'2026-07-01', endDate:'2026-07-10',
    startTime:'08:00', endTime:'13:30', groupCount:10, maxPerGroup:35
  });

  const ph3=DB.insert('phases',{
    programId:prog.id, name:'Phase 3', startDate:'2026-08-01', endDate:'2026-08-10',
    startTime:'08:00', endTime:'13:30', groupCount:10, maxPerGroup:35
  });

  const genDatesList=(sd,ed)=>{
    const dates=[]; let curr=new Date(sd); const last=new Date(ed);
    while(curr<=last){ dates.push(curr.toISOString().slice(0,10)); curr.setDate(curr.getDate()+1); }
    return dates;
  };
  genDatesList(ph1.startDate,ph1.endDate).forEach(d=>DB.insert('operatingDates',{phaseId:ph1.id,date:d}));
  genDatesList(ph3.startDate,ph3.endDate).forEach(d=>DB.insert('operatingDates',{phaseId:ph3.id,date:d}));

  const v1=DB.insert('venues',{phaseId:ph1.id,name:'Ezzat Hall A'});
  const v2=DB.insert('venues',{phaseId:ph1.id,name:'Ezzat Hall B'});
  const v3=DB.insert('venues',{phaseId:ph1.id,name:'Noor Villa'});
  const v4=DB.insert('venues',{phaseId:ph3.id,name:'Burhani Complex Ground'});
  const v5=DB.insert('venues',{phaseId:ph3.id,name:'Burhani Complex Hall 1'});
  const v6=DB.insert('venues',{phaseId:ph3.id,name:'Burhani Complex Hall 2'});

  const addLogin=(itsId,name,role,pw,extra)=>{
    const salt=rndSalt();
    return DB.insert('users',Object.assign({
      itsId,name,role,salt,hash:hashPw(pw,salt),active:true,needsPasswordChange:false
    },extra||{}));
  };

  addLogin('30456117','Program Admin','admin','happycare123');
  addLogin('10000000','Gate Desk Account','checkin','happycare123');

  const ttItems=[
    {start:'08:00',end:'08:30',activity:'Assembly & Duas',location:'Classroom',notes:'Start with morning prayers'},
    {start:'08:30',end:'09:30',activity:'Hifz ul Quran',location:'Quran Room',notes:'Recitation and memorization'},
    {start:'09:30',end:'10:00',activity:'Snack Break',location:'Dining Area',notes:'Healthy snacks provided'},
    {start:'10:00',end:'11:30',activity:'Interactive Learning & Craft',location:'Activity Room',notes:'Creative hands-on tasks'},
    {start:'11:30',end:'12:30',activity:'Adab & Akhlaq',location:'Classroom',notes:'Moral stories and character building'},
    {start:'12:30',end:'13:00',activity:'Lunch & Relax',location:'Dining Area',notes:'Light lunch and free play'},
    {start:'13:00',end:'13:30',activity:'Pickup Prep & Kahani',location:'Classroom',notes:'Storytelling session before departure'}
  ];
  const tpl36=DB.insert('timetableTemplates',{name:'Age 3–6 Schedule',ageCategory:'3–6',items:ttItems});
  const tpl710=DB.insert('timetableTemplates',{name:'Age 7–10 Schedule',ageCategory:'7–10',items:ttItems});

  const setupPhaseGroupsAndStaff=(phase,venues)=>{
    const phaseGroups=[];
    const supName=randName();
    const supIts=String(31000000+phase.id*100+1);
    const supStaff=DB.insert('staff',{
      itsId:supIts,name:supName,role:'Supervisor',mobile:randPhone(),altMobile:randPhone(),phaseId:phase.id,visibleToParents:true
    });
    addLogin(supIts,supName,'supervisor','happycare123',{staffId:supStaff.id});

    for(let i=1;i<=10;i++){
      const catIdx=Math.floor((i-1)/2.6);
      const ageCat=categories[catIdx]||categories[3];
      const ageBrief=categoriesBrief[catIdx]||categoriesBrief[3];
      const g=DB.insert('groups',{
        phaseId:phase.id, name:`Group ${i} (${ageBrief})`, code:`G${phase.id}${String(i).padStart(2,'0')}`,
        ageCategory:ageCat, capacity:phase.maxPerGroup, room:`Room ${i*10}`, status:'ready',
        timetableTemplateId:ageCat.includes('3')?tpl36.id:tpl710.id, venueId:randEl(venues).id
      });
      phaseGroups.push(g);

      const mtName=randName();
      const mtIts=String(32000000+phase.id*100+i);
      const mt=DB.insert('staff',{
        itsId:mtIts,name:mtName,role:'Main Teacher',mobile:randPhone(),altMobile:randPhone(),phaseId:phase.id,visibleToParents:true
      });
      addLogin(mtIts,mtName,'teacher','happycare123',{staffId:mt.id,groupId:g.id});
      DB.insert('groupStaff',{groupId:g.id,slot:'Main Teacher',staffId:mt.id});

      const atName=randName();
      const atIts=String(33000000+phase.id*100+i);
      const at=DB.insert('staff',{
        itsId:atIts,name:atName,role:'Assistant Teacher',mobile:randPhone(),altMobile:randPhone(),phaseId:phase.id,visibleToParents:true
      });
      DB.insert('groupStaff',{groupId:g.id,slot:'Assistant Teacher 1',staffId:at.id});

      const talName=randName();
      const talIts=String(34000000+phase.id*100+i);
      const tal=DB.insert('staff',{
        itsId:talIts,name:talName,role:'Talebaat',mobile:randPhone(),altMobile:randPhone(),phaseId:phase.id,visibleToParents:true
      });
      DB.insert('groupStaff',{groupId:g.id,slot:'Talebaat 1',staffId:tal.id});

      const mdName=randName();
      const md=DB.insert('staff',{
        itsId:'',name:mdName,role:'Maid',mobile:randPhone(),altMobile:'',phaseId:phase.id,visibleToParents:false
      });
      DB.insert('groupStaff',{groupId:g.id,slot:'Maid 1',staffId:md.id});
    }
    return phaseGroups;
  };

  const p1Groups=setupPhaseGroupsAndStaff(ph1,[v1,v2,v3]);
  const p3Groups=setupPhaseGroupsAndStaff(ph3,[v4,v5,v6]);

  const seedPhaseChildren=(phase,groups)=>{
    for(let i=1;i<=320;i++){
      const childIts=String(50000000+phase.id*1000+i);
      const childName=randName();
      const ageYrs=2+Math.floor(Math.random()*8);
      const dob=`20${26-ageYrs-10}-${String(1+Math.floor(Math.random()*11)).padStart(2,'0')}-${String(1+Math.floor(Math.random()*27)).padStart(2,'0')}`;
      const medAlert=Math.random()<0.08?randEl(['Nut Allergy','Asthma','Gluten Intolerance','Lactose Sensitive']):'';
      const c=DB.insert('children',{
        itsId:childIts,name:childName,dob,phaseId:phase.id,active:true,medicalAlert:medAlert,assistNote:''
      });

      let assignedGroup=null;
      if(ageYrs<=3) assignedGroup=groups.find(g=>g.ageCategory.includes('2-3'));
      else if(ageYrs<=4) assignedGroup=groups.find(g=>g.ageCategory.includes('3-4'));
      else if(ageYrs<=5) assignedGroup=groups.find(g=>g.ageCategory.includes('4-5'));
      else assignedGroup=groups.find(g=>g.ageCategory.includes('5-6'))||groups[9];

      if(assignedGroup){
        DB.insert('childGroup',{childId:c.id,groupId:assignedGroup.id,phaseId:phase.id,active:true,assignedAt:todayISO()});
      }

      let parent=null;
      const parentsCount=DB.all('parents').length;
      if(parentsCount>0 && Math.random()<0.20 && i!==1){
        parent=randEl(DB.all('parents'));
      }else{
        const pIts=i===1?'600001001':String(60000000+phase.id*1000+i);
        const pName=i===1?'Sakina Ali':`${randEl(firsts)} ${childName.split(' ')[1]}`;
        parent=DB.insert('parents',{
          itsId:pIts,name:pName,relationship:randEl(['Father','Mother']),mobile:randPhone(),altMobile:randPhone(),email:`${pName.toLowerCase().replace(' ','.')}@example.com`,authorisedPickup:true
        });
        addLogin(pIts,pName,'parent','happycare123',{parentId:parent.id});
      }
      DB.insert('childParent',{childId:c.id,parentId:parent.id,relationship:parent.relationship});
    }
  };

  seedPhaseChildren(ph1,p1Groups);
  seedPhaseChildren(ph3,p3Groups);

  const seedSampleAttendance=(phase)=>{
    const dates=phaseDates(phase.id); if(!dates.length)return;
    const date=dates[0]; const kids=DB.filter('children',c=>c.phaseId===phase.id);
    for(let i=0;i<Math.min(kids.length,100);i++){
      const child=kids[i]; const cg=DB.find('childGroup',x=>x.childId===child.id&&x.phaseId===phase.id&&x.active);
      if(!cg)continue;
      const present=Math.random()<0.93;
      const checkIn=present?`08:${String(10+Math.floor(Math.random()*25)).padStart(2,'0')}`:'';
      const checkOut=present&&Math.random()<0.4?`13:${String(15+Math.floor(Math.random()*20)).padStart(2,'0')}`:'';
      DB.insert('childAttendance',{childId:child.id,phaseId:phase.id,date,groupId:cg.groupId,present,checkIn,checkOut,recordedBy:'Gate Desk Account'});
    }
  };
  seedSampleAttendance(ph1);
  seedSampleAttendance(ph3);

  const parentsList=DB.all('parents');
  for(let i=0;i<8;i++){
    const p=randEl(parentsList); const pc=DB.filter('childParent',cp=>cp.parentId===p.id); if(!pc.length)continue;
    const childId=pc[0].childId; const c=DB.byId('children',childId); const cg=DB.find('childGroup',x=>x.childId===childId&&x.active);
    const q=DB.insert('queries',{
      parentId:p.id, childId, phaseId:c.phaseId, groupId:cg?cg.groupId:null,
      category:randEl(['Medical','Attendance','Timetable','General','Feedback']),
      subject:randEl(['Early Pickup Request','Sore throat query','Late check-in','Lost water bottle','Special diet request']),
      message:'This is a sample parent inquiry message regarding their child’s daily schedule. Please let us know the details.',
      status:randEl(['Submitted','In Review','Responded']), createdAt:new Date().toISOString()
    });
    if(q.status==='Responded'){
      DB.insert('queryResponses',{queryId:q.id,byName:'Teacher Fatema',message:'Sure, we have received your request and have marked it in our system accordingly.',createdAt:new Date().toISOString()});
    }
  }
}
  ...
  $('#app').innerHTML=`
    <header class="appbar">
      ...
      <div class="who" style="display:flex;align-items:center;gap:12px">
        ...
        <button class="iconbtn" onclick="openMenu()" title="Menu">${svg('list')}</button>
        <button class="av" onclick="go(isParent?'p-account':'staff-profile')">${initials(u.name)}</button>
      </div>
    </header>
    <div class="wrap">
...
function restoreData(){ const inp=document.createElement('input'); inp.type='file'; inp.accept='.json,application/json'; inp.onchange=e=>{ const f=e.target.files[0]; if(!f)return; const r=new FileReader(); r.onload=async ()=>{ try{await DB.importJSON(r.result); toast('Data restored'); const p=DB.all('phases')[0]; if(p)setPhaseSilent(p.id); render();}catch(err){toast('Invalid backup file','err');} }; r.readAsText(f); }; inp.click(); }
async function resetDemo(){ if(!confirm('Reset all data back to the demo seed? This cannot be undone.'))return; await DB.wipe(); DB.syncEnabled = false; seed(); DB.syncEnabled = true; if(DB.isCloud){ await DB.saveAllToServer(); } toast('Demo data reset'); const p=DB.all('phases')[0]; setPhaseSilent(p.id); closeModal(); render(); }
...
function todayGroupCard(g,date){
  ...
  return `<button class="gcard s-${r.ready?'ok':'warn'}" onclick="go('group',{id:${g.id},from:'today'})">
  ...
  return {title:'Setup', html:`
    ...
    <div class="card" style="padding:20px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
      <div style="flex:1;min-width:260px">
        <div style="font-weight:800;font-size:15px;color:var(--wine)">Manage System Access</div>
...
function refreshGroupingRight(){ const el=$('#grp-right'); if(!el)return; const gs=phaseGroups(currentPhase().id);
  ...
  el.innerHTML=`<div class="stack">`+gs.map(g=>{ const n=groupChildren(g.id).length; const pct=Math.min(100,n/g.capacity*100); const full=n>=g.capacity;
    return `<div class="card" style="padding:13px">
      <div style="display:flex;justify-content:space-between;align-items:center"><button onclick="go('group',{id:${g.id},from:'grouping'})" style="text-align:left"><div style="font-weight:800;font-size:14.5px">${esc(g.name)}</div><div class="muted" style="font-size:11.5px">${esc(g.code)} · Age ${esc(g.ageCategory)} · ${esc(g.room||'No room')}</div></button>
...
SCREENS.groups=function(){ const ph=currentPhase(); const gs=phaseGroups(ph.id);
  return {title:'Groups', html:`${subhead('Groups',ph.name,'grouping')}
    ...
    <div class="grid g2">${gs.map(g=>{ const r=groupReadiness(g); const n=groupChildren(g.id).length; const st=groupStaffList(g.id).filter(x=>x.staffId).length;
      return `<button class="gcard s-${r.ready?'ok':'warn'}" onclick="go('group',{id:${g.id},from:'groups'})"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><h4>${esc(g.name)}</h4><div class="gm">${esc(g.code)} · Age ${esc(g.ageCategory)} · ${(() => {
    const v = g.venueId ? DB.byId('venues', g.venueId) : null;
    return esc(v ? (g.room ? `${v.name} (${g.room})` : v.name) : (g.room || 'No venue'));
...
SCREENS.group=function(p){ const g=DB.byId('groups',p.id); if(!g)return{title:'Group',html:emptyState('grid','Group not found','','')};
  ...
  return {title:g.name, html:`${subhead(g.name,g.code+' · '+g.ageCategory,p.from||'groups')}
...
/* ============================================================
   USER ACCESS & ROLES
   ============================================================ */
let userSearch = '', userRoleFilter = 'all';
SCREENS['user-access'] = function() {
  if (currentUser().role !== 'admin') {
    return {title: 'Access Denied', html: emptyState('lock', 'Access Denied', 'Only Program Admins can manage user access.', '')};
  }
  return {
    title: 'User Access & Roles',
    html: `
      ${subhead('User Access & Roles', 'Manage system users, login accounts and access levels', 'setup')}
      <div class="btnrow" style="margin-bottom:12px">
        <button class="btn sm" onclick="openUserAccountForm(0)">${svg('plus')} Add User Login</button>
        <button class="btn ghost sm" onclick="openRoleDefinitions()">${svg('shield')} View Role Definitions</button>
      </div>
      <div class="search" style="margin-bottom:10px">${svg('search')}<input id="usr-search" placeholder="Search by name or ITS ID" value="${esc(userSearch)}" oninput="userSearch=this.value;refreshUserList()"></div>
      <div class="filters" style="margin-bottom:12px" id="usr-filters">
        ${['all', 'admin', 'supervisor', 'teacher', 'checkin', 'parent'].map(f => `<button class="fchip ${userRoleFilter === f ? 'on' : ''}" onclick="userRoleFilter='${f}';syncUserFilters();refreshUserList()">${f === 'all' ? 'All Roles' : ROLE_LABEL[f] || (f === 'checkin' ? 'Check-in Desk' : f)}</button>`).join('')}
      </div>
      <div id="usr-list"></div>
    `,
    after: refreshUserList
  };
};

function syncUserFilters() {
  document.querySelectorAll('#usr-filters .fchip').forEach((b, i) => b.classList.toggle('on', ['all', 'admin', 'supervisor', 'teacher', 'checkin', 'parent'][i] === userRoleFilter));
}
...
function refreshUserList() {
  ...
  if (q) {
    list = list.filter(u => (u.name || '').toLowerCase().includes(q) || (u.itsId || '').includes(q));
  }
  
  list.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  
  if (!list.length) {
    el.innerHTML = emptyState('people', 'No users found', 'Adjust filters or add a new user login.', '');
    return;
  }
  
  el.innerHTML = `<div class="muted" style="font-size:12px;margin-bottom:8px">${list.length} user logins</div><div class="rows">` + list.map(u => {
    const active = u.active !== false;
    let desc = ROLE_LABEL[u.role] || u.role;
    if (u.staffId) {
      const s = DB.byId('staff', u.staffId);
      if (s) desc += ` · Linked to Staff: ${s.name}`;
    } else if (u.parentId) {
      const p = DB.byId('parents', u.parentId);
      if (p) desc += ` · Linked to Parent: ${p.name}`;
    }
    
    return `<div class="row">
      <div class="av" style="background:${u.role==='admin'?'var(--wine-100)':(u.role==='parent'?'var(--gold-100)':'var(--blue-100)')};color:${u.role==='admin'?'var(--wine)':(u.role==='parent'?'var(--gold)':'var(--blue)')}">
        ${svg(u.role === 'admin' ? 'shield' : (u.role === 'parent' ? 'user' : (u.role === 'checkin' ? 'door' : 'people')))}
      </div>
      <div class="meta">
        <div class="nm">${esc(u.name || 'Unnamed')} ${u.needsPasswordChange ? '<span class="tag" style="color:var(--warn);border-color:var(--warn-line)">Reset Pending</span>' : ''}</div>
        <div class="sm"><span class="mono">ITS ${esc(u.itsId)}</span> · ${desc}</div>
      </div>
      <div class="end">
        <span class="chip ${active ? 'ok' : 'idle'}">${active ? 'Active' : 'Disabled'}</span>
        <button class="btn ghost sm" style="min-height:30px;padding:4px 8px;min-width:auto;color:var(--warn)" title="Reset password to default (happycare123)" onclick="adminResetDirectUserPassword(${u.id})">${svg('key')}</button>
        <button class="btn ghost sm" style="min-height:30px;padding:4px 8px;min-width:auto" onclick="openUserAccountForm(${u.id})">${svg('edit')}</button>
        ${currentUser().id !== u.id ? `<button class="xbtn" style="width:30px;height:30px" onclick="deleteUserAccount(${u.id})">${svg('trash')}</button>` : ''}
      </div>
    </div>`;
  }).join('') + `</div>`;
}

function adminResetDirectUserPassword(userId) {
  const u = DB.byId('users', userId);
  if (!u) return;
  if (!confirm(`Reset password for ${u.name} back to the default "happycare123"?`)) return;
  const salt = rndSalt();
...
function deleteUserAccount(userId) {
  ...
  if (currentUser().id === u.id) {
    toast('Cannot delete your own admin account', 'err');
    return;
  }
  if (!confirm(`Are you sure you want to delete the user login account for ${u.name}? They will lose access immediately.`)) return;
  DB.remove('users', u.id);
  toast(`User account deleted`);
  refreshUserList();
}

function openRoleDefinitions() {
  openModal('Role Definitions', `
    <div style="display:flex;flex-direction:column;gap:12px">
      <div class="card" style="padding:12px;box-shadow:none;border-color:var(--line-soft)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="chip wine" style="font-size:11px">Admin</span></div>
        <p style="font-size:12.5px;color:var(--ink-soft)">Full system access. Configures programs, phases, venues, groups, assigns children and staff, designs timetables, manages queries, closing tasks, database backups, and user logins.</p>
      </div>
      <div class="card" style="padding:12px;box-shadow:none;border-color:var(--line-soft)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="chip warn" style="font-size:11px">Phase Supervisor</span></div>
        <p style="font-size:12.5px;color:var(--ink-soft)">Phase-wide supervisor level. Monitors daily attendance stats, manages child & staff attendance for all groups, coordinates and resolves parent queries, views reports, and manages daily/phase closing checklists.</p>
      </div>
      <div class="card" style="padding:12px;box-shadow:none;border-color:var(--line-soft)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="chip info" style="font-size:11px">Group Teacher</span></div>
        <p style="font-size:12.5px;color:var(--ink-soft)">Assigned to a specific classroom group. View their own group's children roster, take daily attendance, and directly read and reply to parent communication queries for their children.</p>
      </div>
      <div class="card" style="padding:12px;box-shadow:none;border-color:var(--line-soft)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="chip idle" style="font-size:11px">Check-in / Pickup Desk</span></div>
        <p style="font-size:12.5px;color:var(--ink-soft)">Gate/Desk staff interface. Optimized search panel to quickly look up children by name, student ITS, parent ITS, or mobile, and record checked-in and checked-out timestamps safely.</p>
      </div>
      <div class="card" style="padding:12px;box-shadow:none;border-color:var(--line-soft)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span class="chip ok" style="font-size:11px">Parent</span></div>
        <p style="font-size:12.5px;color:var(--ink-soft)">Limited personal portal. Shows their own linked children, their assigned groups/classrooms, daily group timetables, historical attendance records, and lets them open query tickets to contact the care team.</p>
      </div>
    </div>
  `, `<button class="btn block" onclick="closeModal()">Close</button>`);
}

function openUserAccountForm(id) {
  const u = id ? DB.byId('users', id) : { itsId: '', name: '', role: 'teacher', active: true, staffId: null, parentId: null };
  const staff = DB.all('staff').sort((a,b) => a.name.localeCompare(b.name));
  const parents = DB.all('parents').sort((a,b) => a.name.localeCompare(b.name));
  
  openModal(id ? 'Edit User Login' : 'Add User Login', `
    <div class="field"><label>ITS ID <span class="req">*</span></label><input class="control" id="uf-its" inputmode="numeric" value="${esc(u.itsId)}" placeholder="e.g. 10000003" ${id?'disabled':''}></div>
    <div class="field"><label>Full Name <span class="req">*</span></label><input class="control" id="uf-name" value="${esc(u.name)}" placeholder="e.g. Yusuf Contractor"></div>
    <div class="field"><label>Access Role <span class="req">*</span></label>
      <select class="control" id="uf-role" onchange="onUserRoleChange()">
        <option value="admin" ${u.role==='admin'?'selected':''}>Admin</option>
        <option value="supervisor" ${u.role==='supervisor'?'selected':''}>Phase Supervisor</option>
        <option value="teacher" ${u.role==='teacher'?'selected':''}>Group Teacher</option>
        <option value="checkin" ${u.role==='checkin'?'selected':''}>Check-in / Pickup Desk</option>
        <option value="parent" ${u.role==='parent'?'selected':''}>Parent</option>
      </select>
    </div>
    
    <div class="field" id="uf-link-staff-row" style="display:${(u.role==='teacher'||u.role==='supervisor')?'':'none'}">
      <label>Link to Staff Member</label>
      <select class="control" id="uf-staffId">
        <option value="">— Standalone Login Account —</option>
        ${staff.map(s => `<option value="${s.id}" ${u.staffId===s.id?'selected':''}>${esc(s.name)} (ITS: ${esc(s.itsId)})</option>`).join('')}
      </select>
    </div>
    
    <div class="field" id="uf-link-parent-row" style="display:${u.role==='parent'?'':'none'}">
      <label>Link to Parent Profile</label>
      <select class="control" id="uf-parentId">
        <option value="">— Standalone Login Account —</option>
        ${parents.map(p => `<option value="${p.id}" ${u.parentId===p.id?'selected':''}>${esc(p.name)} (ITS: ${esc(p.itsId)})</option>`).join('')}
      </select>
    </div>
    
    <label style="display:flex;align-items:center;gap:9px;font-size:13px;font-weight:600;cursor:pointer;margin-top:12px">
      <input type="checkbox" id="uf-active" ${u.active!==false?'checked':''}> Account Enabled
    </label>
    ${!id ? '<p class="hint" style="margin-top:8px">New accounts default to password <b>happycare123</b> and require a password change on first sign-in.</p>' : ''}
  `, `
    <button class="btn ghost block" onclick="closeModal()">Cancel</button>
    <button class="btn block" onclick="saveUserAccount(${id||0})">Save Account</button>
  `);
}

function onUserRoleChange() {
  const role = $('#uf-role').value;
  $('#uf-link-staff-row').style.display = (role === 'teacher' || role === 'supervisor') ? '' : 'none';
  $('#uf-link-parent-row').style.display = (role === 'parent') ? '' : 'none';
}

function saveUserAccount(id) {
  const itsId = val('uf-its').trim();
  const name = val('uf-name');
  const role = $('#uf-role').value;
  const active = checked('uf-active');
  
  if (!itsId || !name) {
    toast('ITS ID and Name are required', 'err');
    return;
  }
  
  if (!id && DB.find('users', u => u.itsId === itsId)) {
    toast('A user with this ITS ID already exists', 'err');
    return;
  }
  
  const staffSelect = $('#uf-staffId');
  const parentSelect = $('#uf-parentId');
  const staffId = (role === 'teacher' || role === 'supervisor') && staffSelect && staffSelect.value ? Number(staffSelect.value) : null;
  const parentId = (role === 'parent') && parentSelect && parentSelect.value ? Number(parentSelect.value) : null;
  
  const rec = { name, role, active, staffId, parentId };
  
  if (id) {
    DB.update('users', id, rec);
    toast('User account updated');
  } else {
    const salt = rndSalt();
    DB.insert('users', Object.assign({
      itsId,
      salt,
      hash: hashPw('happycare123', salt),
      needsPasswordChange: true
...
SCREENS.timetables=function(){ const tpl=DB.all('timetableTemplates'); const ph=currentPhase();
  return {title:'Timetables', html:`${subhead('Timetable templates','Reusable daily schedules','grouping')}
...
SCREENS['attendance-child']=function(p){ const ph=currentPhase(); const date=state.dateISO; const gs=phaseGroups(ph.id);
  ...
  return {title:'Child Attendance', html:`${subhead('Child attendance',fmtDate(date),p&&p.group?'group':'today',p&&p.group?{id:p.group}:{})}
...
SCREENS['attendance-staff']=function(){ const ph=currentPhase(); const date=state.dateISO; const gs=phaseGroups(ph.id);
  ...
  return {title:'Staff Attendance', html:`${subhead('Staff attendance',fmtDate(date),'today')}
...
function searchPeople(q){ q=q.toLowerCase().trim(); if(!q)return[]; const ph=currentPhase(); const set=new Map();
  ...
  return [...set.values()]; }
let ciQuery='';
SCREENS.checkin=function(){ return {title:'Child Check-in', html:`${subhead('Child check-in',fmtDate(state.dateISO),'today')}
...
SCREENS.checkout=function(){ return {title:'Child Check-out', html:`${subhead('Child check-out',fmtDate(state.dateISO),'today')}
...
SCREENS.queries=function(){ const ph=currentPhase();
  ...
  return {title:'Parent Queries', html:`${subhead('Parent queries',ph.name,'today')}
...
SCREENS.closing=function(){ const ph=currentPhase(); const date=state.dateISO; const gs=phaseGroups(ph.id);
  ...
  return {title:'Daily Closing', html:`${subhead('Daily closing',fmtDate(date),'today')}
...
SCREENS.phaseclosing=function(){ const ph=currentPhase(); const gs=phaseGroups(ph.id); const dates=phaseDates(ph.id);
  ...
  return {title:'Phase Closing', html:`${subhead('Phase closing',ph.name,'today')}
...
async function boot(){
  ...
  if (appEl) {
    appEl.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;font-family:var(--font);background:var(--cream);color:var(--ink)">
      <div style="font-size:18px;font-weight:600;margin-bottom:8px">Happy Care · Istefada Ilmiyah</div>
      <div style="font-size:14px;color:var(--ink-soft)">Connecting to cloud database...</div>
    </div>`;
  }
  await DB.loadServer();
  const wasEmpty = DB.all('meta').length === 0;
  if (wasEmpty) {
    DB.syncEnabled = false;
    seed();
    DB.syncEnabled = true;
    if (DB.isCloud) {
      await DB.saveAllToServer();
    }
  } else {
    seed();
  }
...
restoreSession();
  ...
  if(u){
    ...
    if(p)setPhaseSilent(p.id);
    state.route=u.role==='parent'?'p-children':'today';
  }
  render();
}
boot();