...
async function main() {
  ...
  const backupPath = path.join(__dirname, '..', 'happy-care-backup.json');
  if (!fs.existsSync(backupPath)) {
    console.error(`Error: Backup file not found at ${backupPath}`);
    console.error('Please save your browser data backup as "happy-care-backup.json" in the root directory.');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!backupData._seq) {
      throw new Error('Missing sequence field in backup JSON.');
    }
  } catch (error) {
  ...
  // 1. Fetch current tables in Teable Base
  let existingTables = [];
  try {
  ...
  // 2. Create missing tables and columns in Teable
  ...
  // 3. Upload Backup data into Teable
  ...
  // 4. Write final updated mapping to .env file
  const dbUrlLine = process.env.DATABASE_URL ? `DATABASE_URL="${process.env.DATABASE_URL}"\n` : '';
  const envContent = `${dbUrlLine}TEABLE_API_URL="${apiBaseUrl}"
...
`;
  ...
  console.log('All local backup data has been shifted to Teable!');
