...
const DB={
  ...
  load(){ try{const raw=localStorage.getItem(this.key); this.data=raw?JSON.parse(raw):null;}catch(e){this.mem=true;this.data=null;}
    if(!this.data){this.data={_seq:{}}; this.tables.forEach(t=>this.data[t]=[]);} if(!this.data._seq)this.data._seq={}; this.parseJSONFields(); },
  ...
  importJSON(txt){ const d=JSON.parse(txt); if(!d._seq)throw new Error('Not a Happy Care backup'); this.data=d; this.parseJSONFields(); this.save(); },
  parseJSONFields(){
    if(!this.data)return;
